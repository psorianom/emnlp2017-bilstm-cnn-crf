...
class BiLSTM:
    ...
    def computeScore(self, modelName, devMatrix, testMatrix):
        if self.labelKeys[modelName].endswith('_BIO') or self.labelKeys[modelName].endswith('_IOBES') or self.labelKeys[
            modelName].endswith('_IOB'):
    ...
    def computeF1(self, modelName, sentences, save_predictions=False):
        ...
        pre, rec, f1, label_correct, label_pred = BIOF1Validation.compute_f1(predLabels, correctLabels, idx2Label, 'O',
                                                                             encodingScheme)
        pre_b, rec_b, f1_b, label_correct, label_pred = BIOF1Validation.compute_f1(predLabels, correctLabels, idx2Label,
                                                                                   'B', encodingScheme)
